...
def GnomeSortCPU(a): #Worst case: n^2, best case: n, average: n^2
    n=len(a)
    index =0
    iter=0
    while index<n:
...
def GnomeSort(a): #Worst case: n^2, best case: n, average: n^2
    ...
    while index<n:
        ...
        if a[index]>=a[index-1]:
            ...
            index-=1
    return iter
def BubbleSortCPU(a): #Worst case: n^2, best case: n, average: n^2
    iter=0
    swapped = False
...
def BubbleSort(a): #Worst case: n^2, best case: n, average: n^2
    ...
    for i in range(1, len(a)):
        ...
        if not swapped:
            return iter
    return iter
def InsertionSortCPU(a): #Worst case: n^2 Best case: n Average: n^2
    iter=0
    for i in range(1,len(a)):
        key = a[i]
        j= i-1
...
def SelectionSortCPU(a): #Worst case: n^2 Best case: n Average: n^2
    iter=0
...
def CocktailSortCPU(a): #Worst case: n^2 Best case: n Average: n^2
    iter=0
    swapped = True
    l=0
    r=len(a)-1
...
def QuickSortCPU(a,low,high): #Worst case: n^2 Best case: n*log(n) Average: n*log(n)
    iter=0
    if low<high:
        iter+=1
...
def MergeSortCPU(a,l,r): #Worst case: O(n*log(n)) Best case: Omega(n*log(n)) Average: Theta(n*log(n))
    iter=0
    if l<r:
        iter+=1
        m=l+(r-l)//2
...
def HeapSortCPU(a): #Worst case: n*log(n) Best case: n*log(n) or n (equal keys) Average: n*log(n)
    iter=0
    n=len(a)
...
def TimSortCPU(a): #Worst case: n*log(n) Best case: n Average: n*log(n)
    iter=0
    n=len(a)
...
@jit(target_backend='cuda')
def TimSort(a): #Worst case: n*log(n) Best case: n A verage: n*log(n)
    iter = 0
    n = len(a)
    minRun = CalculateMinRun(n)
...
for i in range(4,8): #Half sorted, half random (except nums, they repeat)
    ...
    for j in range(len(intArrays[i])//2):
        ...
        strArrays[i][j]=str(j)
#        if j!=0:
#            dateArrays[i][j]=dateArrays[i][j-1]+dt.date(0,0,1)
...
testA1=strArrays[3].copy()
testA2=strArrays[3].copy()
testA3=[0]*500000
for i in range (len(testA3)):
    testA3[i]=rng.randint(0,1000)

start1=timer()
QuickSort(testA2.copy(),0,len(testA2)-1)
print(timer()-start1)
